...
const Projects = () => {
  const projects = [
    {
      title: "SyntaxSniffer",
      description: "AI-powered code reviewer using React.js, TailwindCSS, and Gemini API to analyze and provide intelligent feedback on code quality and optimization.",
    ...
    {
      title: "TaskFlow",
      description: "Task management web app built with React and Tailwind CSS featuring dynamic task filters and real-time UI updates.",
    ...
    <section id="projects" className="py-16 sm:py-20 bg-white dark:bg-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12 sm:mb-16">
          ...
          <p className="text-lg sm:text-xl text-gray-600 dark:text-gray-300">Explore the projects I've conquered</p>
        </div>

        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8">
          {projects.map((project, index) => (
            <div key={index} className="bg-gradient-to-br from-blue-500 to-purple-600 p-1 rounded-2xl shadow-xl">
              <div className="bg-white dark:bg-gray-700 p-6 sm:p-8 rounded-xl h-full">
                <div className="flex flex-col sm:flex-row items-start justify-between mb-6 space-y-4 sm:space-y-0">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white pr-0 sm:pr-4">{project.title}</h3>
                  <div className="flex flex-row sm:flex-col space-x-2 sm:space-x-0 sm:space-y-2 w-full sm:w-auto justify-start sm:justify-end">
                    ...
                    <span className={`px-3 sm:px-4 py-1 rounded-full text-xs sm:text-sm font-bold shadow-sm text-center whitespace-nowrap ${
                ...
                <div className="mb-6">
                  <h4 className="font-bold text-gray-800 dark:text-white mb-3 text-sm sm:text-base">🔧 Power-Ups Used:</h4>
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.map((tech, idx) => (
                      <span key={idx} className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium shadow-sm">
                        {tech}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mb-6">
                  <h4 className="font-bold text-gray-800 dark:text-white mb-3 text-sm sm:text-base">⭐ Achievements:</h4>
                  <ul className="space-y-2">
                    {project.highlights.map((highlight, idx) => (
                      <li key={idx} className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 flex items-start leading-relaxed">
                        <Trophy className="text-yellow-500 mr-2 mt-1 flex-shrink-0" size={14} />
                        <span>{highlight}</span>
                      </li>
                    ))}
                  </ul>
